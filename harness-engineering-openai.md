...
## 根目录文件
* **AGENTS.md** - AI Agent 角色定义与协作指令
* **ARCHITECTURE.md** - 系统整体架构设计
* **DESIGN.md** - 通用设计准则
* **FRONTEND.md** - 前端技术栈与开发规范
* **PLANS.md** - 高层级路线图与计划
* **PRODUCT_SENSE.md** - 产品逻辑与用户体验深度思考
* **QUALITY_SCORE.md** - 质量度量标准与评分体系
* **RELIABILITY.md** - 系统可靠性与稳定性方案
* **SECURITY.md** - 安全审计与防护策略
...
### 🎨 design-docs/ (设计文档)
* `index.md` - 设计文档索引
* `core-beliefs.md` - 核心设计理念/哲学
* *... (其他具体模块设计)*

### 🚀 exec-plans/ (执行计划)
* **active/** - 正在进行的任务/Sprint
* **completed/** - 已归档的完成记录
* `tech-debt-tracker.md` - 技术债追踪表

### 🤖 generated/ (自动生成)
* `db-schema.md` - 自动生成的数据库关系图/表结构

### 📝 product-specs/ (产品规格)
* `index.md` - 规格说明索引
* `new-user-onboarding.md` - 新用户引导流程详述
* *... (其他功能规格书)*

### 📚 references/ (技术参考 - 优化 LLM 上下文)
* `design-system-reference-llms.txt` - 设计系统参考 (LLM 友好版)
* `nixpacks-llms.txt` - Nixpacks 构建参考
* `uv-llms.txt` - UV 包管理参考
* *... (其他技术栈参考资料)*
...
The rule: within each business domain (e.g. App Settings), code can only depend "forward" through a fixed set of layers:

```
Types → Config → Repo → Service → Runtime → UI
```

Cross-cutting concerns (auth, connectors, telemetry, feature flags) enter through a single explicit interface: **Providers**. Anything else is disallowed and enforced mechanically.
...
Instead, we started encoding what we call "golden principles" directly into the repository and built a recurring cleanup process. These principles are opinionated, mechanical rules that keep the codebase legible and consistent for future agent runs. For example:
