...
6. **Documentation infrastructure check:** Check if `docs/.doc-meta.json` exists. If it does not
   and the plan introduces new architectural components, modules, or significant interfaces:
   suggest running `/doc-init` as part of the implementation plan to establish the project knowledge
   base. Frame as: "This plan introduces [N] new components — a `docs/` knowledge base would help
   AI agents (and future contributors) navigate the growing architecture. Consider running `/doc-init`
   after implementation." This is advisory — note it in the "NOT in scope" section if deferred.
